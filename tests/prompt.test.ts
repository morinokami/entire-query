...
describe("eq prompt", () => {
  test("returns the full prompt.txt content", async () => {
    ...
    expect(r.output).toEqual({
      ...
      document: {
        ...
        prompt: "Please review src/foo.ts thoroughly.",
      },
    });
  });

  test("missing prompt.txt → prompt: null (no error)", async () => {
    ...
    expect(r.output.document?.prompt).toBeNull();
